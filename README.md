...
6. Метод получения списка сегментов в котором состоит пользователь.
...
2. Создать пользователя (некорректный запрос) http://localhost:8080/main/adduser POST:
  - INPUT
  ```json
  {
    "user_number":-4
...
4. Создать сегмент с использованием процентов - количество пользователей автоматически попадающих в сегмент (корректный запрос) http://localhost:8080/main/addsegment POST:
...
6. Создать сегмент с использованием процентов - количество пользователей автоматически попадающих в сегмент (некорректный запрос) http://localhost:8080/main/addsegment POST:
  - INPUT
  ```json
  {
  ...
  ```json
  {
    "Message": "Fix Percent ,pls" // проценты не могут быть меньше 0, не могут быть больше 100, 
  }
  ```
...
12. Удалить сегменты у пользователя (некорректный запрос) http://localhost:8080/main/delsegmentuser DELETE:
  ...
  ```json
  {
    "Status": "no such data" // пользователя с таким UserID - нет
      // в этом примере в отличие от предыдущего если указан элемент списка, который не существует в Usersslug - просто пропускается
  }
  ```
...
16. Получить ссылку на csv файл с отчетом об удалении/добавлении сегментов пользователю (некорректный запрос):
  ...
  ```json
  {
    // http://localhost:8080/static/otchet.csv  при некоректном запросе будет также выдаваться сслыка, но файл будет пустым
    // вводим ссылку в браузере - загружается файл
  }
  ```
